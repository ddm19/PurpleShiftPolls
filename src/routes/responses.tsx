...
type ResponsesByUser = Record<string, {
...
function ResponsesPage() {
    ...
    const { byQuestion, byUser } = useMemo(() => processData(responses, questions), [responses, questions]);
    const users = useMemo(() => Object.entries(byUser).sort((a, b) => b[1].date.getTime() - a[1].date.getTime()), [byUser]);
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
            <h1 className="text-3xl font-bold neon-text-purple tracking-widest mb-6">Análisis de Respuestas</h1>

            <div className="flex gap-2 mb-6 border-b border-border/40">
                <button onClick={() => setView("summary")} className={`px-4 py-2 text-sm ${view === 'summary' ? 'neon-text-blue border-b-2 border-blue-500' : 'text-muted-foreground'}`}>Resumen</button>
                <button onClick={() => setView("users")} className={`px-4 py-2 text-sm ${view === 'users' ? 'neon-text-blue border-b-2 border-blue-500' : 'text-muted-foreground'}`}>Por Usuario</button>
            </div>
                    ...
                    <div className="md:col-span-1">
                        ...
                        <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-2">
                            {users.map(([userId, data]) => (
                                <button key={userId} onClick={() => setSelectedUser(userId)} className={`w-full text-left panel-cyber p-3 transition-all ${selectedUser === userId ? 'neon-border-purple' : 'hover:bg-primary/10'}`}>
...
function AnalyticsSummary({ questions, byQuestion, totalUsers }: { questions: Question[], byQuestion: ResponsesByQuestion, totalUsers: number }) {
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="panel-cyber p-4">
                    ...
                    <p className="text-3xl font-bold neon-text-purple">{Object.values(byQuestion).flat().length}</p>
                </div>
            </div>
            ...
            {questions.map(q => (
                <div key={q.id} className="panel-cyber p-6">
                    ...
                    <p className="text-xs text-muted-foreground mb-4">#{q.order} · {q.type.replace("_", " ")} · {byQuestion[q.id]?.length ?? 0} respuestas</p>
...
function QuestionAnalytics({ question, responses }: { question: Question, responses: Response[] }) {
                ...
                {bucketCounts.map((count, i) => {
                    ...
                    return (
                        <div key={i} className="flex-1 flex flex-col items-center justify-end group">
                            <div className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100">{count}</div>
                            <div className="w-full bg-purple-500/50 hover:bg-purple-400" style={{ height: `${height}%` }}></div>
    ...
    return <p className="text-sm text-muted-foreground">// No hay visualización para este tipo de pregunta.</p>;
}

function UserDetailView({ user, questions }: { user: ResponsesByUser[string], questions: Question[] }) {
    return (
        <div className="panel-cyber p-6">
            <h2 className="text-lg font-bold mb-1">Respuestas del Participante</h2>
            <p className="text-xs font-mono text-muted-foreground mb-6">{Object.keys(user.responses)[0]}</p>
